...
class mainUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btnScore = QPushButton('Skor Tertinggi', self)
        self.btnScore.clicked.connect(self.connectHOF)
        self.btnEdit = QPushButton('Edit Soal', self)
        ...
        self.mainLayout.addWidget(self.btnScore)
...
class highscore(QMainWindow):
    ...
    def halloffame(self):
        ...
        lblScore = QLabel("Hall of Fame", self)
        lblScore.setFont(font)
        lblScore.setAlignment(Qt.AlignCenter)
        self.tableView = QTableWidget()
        self.tableView.setColumnCount(2)
        self.tableView.setHorizontalHeaderLabels(["Nama","Highscore"])
        ...
        def loaddata():
            with con.cursor() as x:
                query = "SELECT * FROM skor ORDER BY skor_catat DESC"
                x.execute(query)
                hasil = x.fetchall()
                print(hasil)
                rowsCount = len(hasil)
                ...
                tableRow = 0
                for x in hasil:
                    self.tableView.setItem(tableRow, 0, QTableWidgetItem(x[1]))
                    self.tableView.setItem(tableRow, 1, QTableWidgetItem(str(x[2])))
                    tableRow += 1
        loaddata()
